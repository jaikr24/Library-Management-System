...
public class ReturnController implements Initializable {
    ...
    void save(ActionEvent event) {
        ...
        try {
            ...
            PreparedStatement st = con.prepareStatement("update lb set r_date = SYSDATE() where stud_id = ? and b_id = ?");
            st.setInt(1, sid.getValue());
            st.setInt(2, bid.getValue());
            st.execute();
            PreparedStatement st2 = con.prepareStatement("update book set copies= copies+1 where b_id = ?");
    ...
    public void initialize(URL url, ResourceBundle rb) {
        ...
        sid.getSelectionModel().selectedItemProperty().addListener((v, old, new_i) ->
        {
            ...
            try {
                Connection con = DriverManager.getConnection("jdbc:mysql://localhost:3306/demo", "root", "123456");
                PreparedStatement st = con.prepareStatement("select b_id from lb where stud_id=? and r_date is null");
                st.setInt(1, new_i);
